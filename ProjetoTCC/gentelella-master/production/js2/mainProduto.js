...
btnElement.onclick = function() {
	for (var i = 1; i >= 0; i++) {
		if (document.querySelector('table.tabela tbody#tabela_Dados tr#edit' + i)) {
			...
			linkEdit.setAttribute('href', 'Edit_Produto.php?cod=' + codValue);
			...
			//linkExcluir.setAttribute('action', 'Res_Delet_Produto.php?cod=' + codValue);
...
function linkar(item, indice) {
	item.onclick = function() {
		form.setAttribute('action', 'Res_Delet_Produto.php?cod=' + sla[indice]);
	}
}
