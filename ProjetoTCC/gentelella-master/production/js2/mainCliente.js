...
btnElement.onclick = function() {
	for (var i = 1; i >= 0; i++) {
		if (document.querySelector('table.tabela tbody#tabela_Dados tr#edit' + i)) {
			...
			linkEdit.setAttribute('href', 'Edit_Cliente.php?cod=' + codValue);
			...
			//linkExcluir.setAttribute('action', 'Res_Delet_Cliente.php?cod=' + codValue);
...
function linkar(item, indice) {
	item.onclick = function() {
		form.setAttribute('action', 'Res_Delet_Cliente.php?cod=' + sla[indice]);
	}
}
